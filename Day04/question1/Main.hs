...
--
calculate :: Map.Map Integer [Integer] -> [(Integer, Int, Integer)]
calculate m = map (calculate') $ Map.toList m
  where
    calculate' :: (Integer, [Integer]) -> (Integer, Int, Integer)
    calculate' (guard, minutes) = (guard, length minutes, frequent minutes)
      where
        frequent :: [Integer] -> Integer
        frequent minutes = fst $ maximumBy (\a b -> compare (snd a) (snd b)) $ Map.toList $ foldl (\acc a -> Map.insertWith (+) a 1 acc) (Map.empty) minutes


solution :: [Log] -> (Integer, Int, Integer)
solution cs = maximumBy (\(a,b,c) (a',b',c') -> compare b b') $ calculate $ pack cs
